...
input_id_nodo = ttk.Combobox(frame, values=[0], textvariable=id_node, state="readonly")
input_id_nodo.grid(row=0, column=1)
input_id_nodo.current(0)

text_mac_nodo = Label(frame, text="Direción MAC:").grid(row=0, column=2, padx=8)
input_mac_nodo = Entry(frame, textvariable=mac_node)
input_mac_nodo.grid(row=0, column=3)

text_status_node = Label(frame, text="Estado:").grid(row=0, column=4, padx=8)
input_status_node = ttk.Combobox(frame, values=["Activo", "Inactivo"], textvariable=status_node, state='readonly')
input_status_node.grid(row=0, column=5)
input_status_node.current(0)

text_latitude_node = Label(frame, text="Latitud:").grid(row=0, column=6, padx=8)
input_latitude_node = Entry(frame, textvariable=latitude_node)
input_latitude_node.grid(row=0, column=7)

text_longitude_node = Label(frame, text="Longitud:").grid(row=0, column=8, padx=8)
input_longitude_node = Entry(frame, textvariable=longitude_node)
input_longitude_node.grid(row=0, column=9)

text_threshold = Label(frame_variables, text="Umbral de alerta:").grid(row=0, column=0, padx=8)
input_threshold = Entry(frame_variables, textvariable=threshold)
input_threshold.grid(row=0, column=1)

text_interval = Label(frame_variables, text="Periodo de muestreo:").grid(row=0, column=2, padx=8)
